> 内容来源于：[https://liaoxuefeng.com/books/java/collection/properties/index.html](https://liaoxuefeng.com/books/java/collection/properties/index.html)
...
调用`getProperty()`获取配置时，如果key不存在，将返回null。我们还可以提供一个默认值，这样，当key不存在的时候，就返回默认值。
