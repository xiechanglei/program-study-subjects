> 内容来源于：[https://liaoxuefeng.com/books/java/generics/what-is-generics/index.html](https://liaoxuefeng.com/books/java/generics/what-is-generics/index.html)
...
要解决上述问题，我们可以为String单独编写一种`ArrayList`：
...
然而，新的问题是，如果要存储`Integer`，还需要为Integer单独编写一种`ArrayList`：
...
这是不可能的，JDK的`class`就有上千个，而且它还不知道其他人编写的`class`。
...
实际上，编译器为了避免这种错误，根本不允许把`ArrayList<Integer>`转型为`ArrayList<Number>`。
