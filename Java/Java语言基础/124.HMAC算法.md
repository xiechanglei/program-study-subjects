> 内容来源于：[https://liaoxuefeng.com/books/java/security/hmac/index.html](https://liaoxuefeng.com/books/java/security/hmac/index.html)
...
因此，HmacMD5可以看作带有一个安全的`key`的MD5。使用HmacMD5而不是用MD5加salt，有如下好处：
...
为了保证安全，我们不会自己指定key，而是通过Java标准库的`KeyGenerator`生成一个安全的随机的key。下面是使用HmacMD5的代码：
...
有了Hmac计算的哈希和`SecretKey`，我们想要验证怎么办？这时，`SecretKey`不能从KeyGenerator生成，而是从一个`byte[]`数组恢复：
