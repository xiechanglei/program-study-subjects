> 内容来源于：[https://liaoxuefeng.com/books/java/collection/queue/index.html](https://liaoxuefeng.com/books/java/collection/queue/index.html)
...
注意：不要把null添加到队列中，否则`poll()`方法返回null时，很难确定是取到了null元素还是队列为空。

接下来我们以`poll()`和`peek()`为例来说说"获取并删除"与"获取但不删除"的区别。对于Queue来说，每次调用`poll()`，都会获取队首元素，并且获取到的元素已经从队列中被删除了：
