> 内容来源于：[https://liaoxuefeng.com/books/java/unit-test/exception-test/index.html](https://liaoxuefeng.com/books/java/unit-test/exception-test/index.html)
...
上述`->`语法就是函数式接口的实现代码，我们会在后面详细介绍。现在，我们只需要通过这种固定的代码编写能抛出异常的语句即可。
...
1. 测试异常可以使用`assertThrows()`，期待捕获到指定类型的异常；
2. 对可能发生的每种类型的异常都必须进行测试。