> 内容来源于：[https://liaoxuefeng.com/books/java/io/filter/index.html](https://liaoxuefeng.com/books/java/io/filter/index.html)
...
如果要给`FileInputStream`添加3种功能，至少需要3个子类。这3种功能的组合，又需要更多的子类：
...
注意到在叠加多个`FilterInputStream`，我们只需要持有最外层的`InputStream`，并且，当最外层的`InputStream`关闭时（在try(resource)块的结束处自动关闭），内层的`InputStream`的`close()`方法也会被自动调用，并最终调用到最核心的"基础"`InputStream`，因此不存在资源泄露。