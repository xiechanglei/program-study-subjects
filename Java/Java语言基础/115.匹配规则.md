> 内容来源于：[https://liaoxuefeng.com/books/java/reg-exp/match-rules/index.html](https://liaoxuefeng.com/books/java/reg-exp/match-rules/index.html)
...
如果想匹配非ASCII字符，例如中文，那就用`\u####`的十六进制表示，例如：`a\u548cc`匹配字符串"a和c"，中文字符"和"的Unicode编码是548c。
...
用`.`可以匹配任意字符，这个范围开得有点大。如果我们只想匹配0~9这样的数字，可以用`\d`匹配。例如，正则表达式`00\d`可以匹配：
...
如果我们想指定匹配n~m个字符怎么办？用修饰符`{n,m}`就可以。`A\d{3,5}`可以匹配：
