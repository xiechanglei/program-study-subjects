> 内容来源于：[https://liaoxuefeng.com/books/java/datetime/date-calendar/index.html](https://liaoxuefeng.com/books/java/datetime/date-calendar/index.html)
...
`1574208900`表示从1970年1月1日零点GMT时区到该时刻一共经历了`1574208900`秒，换算成伦敦、北京和纽约时间分别是：
...
注意`getYear()`返回的年份必须加上1900，`getMonth()`返回的月份是0~11分别表示1~12月，所以要加1，而`getDate()`返回的日期范围是1~31，又不能加1。
...
上述代码在不同的语言环境会打印出类似`Sun Sep 15, 2019`这样的日期。可以从JDK文档查看详细的格式说明。一般来说，字母越长，输出越长。以`M`为例，假设当前月份是9月：
...
`Date`对象有几个严重的问题：它不能转换时区，除了`toGMTString()`可以按GMT+0:00输出外，`Date`总是以当前计算机系统的默认时区为基础进行输出。此外，我们也很难对日期和时间进行加减，计算两个日期相差多少天，计算某个月第一个星期一的日期等。
