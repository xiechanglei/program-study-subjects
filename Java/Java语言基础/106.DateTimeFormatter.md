> 内容来源于：[https://liaoxuefeng.com/books/java/datetime/datetime-formatter/index.html](https://liaoxuefeng.com/books/java/datetime/datetime-formatter/index.html)
...
1. 对`ZonedDateTime`或`LocalDateTime`进行格式化，需要使用`DateTimeFormatter`类；
2. `DateTimeFormatter`可以通过格式化字符串和`Locale`对日期和时间进行定制输出。