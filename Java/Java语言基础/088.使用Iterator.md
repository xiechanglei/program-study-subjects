> 内容来源于：[https://liaoxuefeng.com/books/java/collection/iterator/index.html](https://liaoxuefeng.com/books/java/collection/iterator/index.html)

Java的集合类都可以使用for each循环，List、Set和 Queue会迭代每个元素，Map会迭代每个key。以List为例：
...
例如，我们虽然知道ArrayList在内部是以数组形式存储元素，并且，它还提供了`get(int)`方法。虽然我们可以用for循环遍历：
...
这里的关键在于，集合类通过调用`iterator()`方法，返回一个Iterator对象，这个对象必须自己知道如何遍历该集合。
